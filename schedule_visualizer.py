...
def _add_cons_line(
    ...
    *,
    ...
    meta_station: int,
    row: int,
    width_px: int = CONS_LINE_WIDTH_PX,
    type_label: str = "",
):
    ...
    fig.add_trace(
        go.Bar(
            ...
            hoverinfo="skip",
            showlegend=False,
            legendgroup="cons_outline",
            name="Consumption (outline)",
...
def plot_schedule(
    ...
    *,
    ...
    show: bool = True,
    utilization_plot_top: bool = False,
) -> go.Figure:
            ...
            if not any_present:
                for k in K:
                    for e in range(U.get(k, 0)):
                        ...
                        if any(
                            iv is not None and _iv_present(solution, iv) for iv in hits
                        ):
                            any_present = True
                            break
                    if any_present:
        ...
        for s in S:
            ...
            if iv is not None and _iv_present(solution, iv):
                ...
                order_intervals_by_station[s].append(
                    (
                        _iv_start(solution, iv),
                        _iv_end(solution, iv),
                        o,
                    )  # (start, end, payload=order_id)
                )
    ...
    if I_os_lane:
        ...
        for o in O:
            ...
            try:
                ln_sel = next(
                    ln for ln in L if _iv_present(solution, I_os_lane[(o, s_sel, ln)])
                )
            except StopIteration:
                raise RuntimeError(
                    f"No lane selected for order {o} at station {s_sel}. Check model constraints."
                )
            assign[o] = (s_sel, ln_sel)
    else:
        # v3 logic: no lanes in model, pack them visually
        print(
            "[Visualizer] Using v3 logic: I_os_lane not found. Packing orders into lanes."
        )
        for s in stations:
            # Pack orders for this station
            packed_orders = _pack_lanes(
                order_intervals_by_station[s]
            )  # list of (st, en, o, lane)
            for st, en, o, ln in packed_orders:
                if ln >= len(L) and len(L) > 0:  # Check vs. model capacity
                    print(
                        f"[Visualizer] Warning: Order {o} (at S{s}) packed into visual lane {ln}, "
        ...
        for e in range(U.get(k, 0)):
            for s in S:
                ...
                if ivf is not None and _iv_present(solution, ivf):
                    robot_events.extend(
                        [(_iv_start(solution, ivf), 1), (_iv_end(solution, ivf), -1)]
                    )
                if ivr is not None and _iv_present(solution, ivr):
                    robot_events.extend(
                        [(_iv_start(solution, ivr), 1), (_iv_end(solution, ivr), -1)]
                    )
    ...
    for t, change in robot_events:
        ...
        peak_robots = max(peak_robots, current_robots)
        last_time = t

    avg_robots = (
        robot_active_time / max(makespan_calc, last_time)
        if max(makespan_calc, last_time) > 0
        else 0.0
    )
    ...
    sku_refetch_ratio = (
        sum(sku_fetches.values()) / len(sku_fetches) if sku_fetches else 0.0
    )
    ...
    for o in O:
        for s in S:
            ...
            if iv is not None and _iv_present(solution, iv):
                ...
                schedule_first = (
                    st if schedule_first is None else min(schedule_first, st)
                )
                schedule_last = en if schedule_last is None else max(schedule_last, en)

    active_window = (
        (schedule_last - schedule_first)
        if (schedule_first is not None and schedule_last != schedule_first)
        else 1
    )
    total_lane_capacity_time = (
        len(stations) * len(L) * active_window if len(L) > 0 else 1
    )
    ...
    total_subplots = (
        len(stations) * 3
    ) - 1  # Gantt + Util + Gap for all except last station
    ...
    for s_idx, s in enumerate(stations):
        ...
        for ln in L:
            ...
            if (
                not has_order_on_lane and len(L) > 0
            ):  # If L=0, we must add at least one lane
                if not any(
                    assign.get(o) == (s, 0) for o in O
                ):  # ...unless we are packing to lane 0
                    continue
                    ...
                    if include_consumptions:
                        for k in orders_req[o]:
                            ...
                            if _iv_present(solution, Civ):
                                cs, ce = (
                                    _iv_start(solution, Civ),
                                    _iv_end(solution, Civ),
                                )
                                _add_cons_line(
                                    fig,
                                    y=y_lane,
    ...
    for s_idx, s in enumerate(stations):
        ...
        for o in O:
            ...
            if iv is not None and _iv_present(solution, iv):
                events["order"].extend(
                    [(_iv_start(solution, iv), 1), (_iv_end(solution, iv), -1)]
                )
            ...
            for k in K:
                for e in range(U.get(k, 0)):
                    ...
                    if ivp is not None and _iv_present(solution, ivp):
                        events["pick"].extend(
                            [
                                (_iv_start(solution, ivp), 1),
                                (_iv_end(solution, ivp), -1),
                            ]
                        )
        ...
        for t in sorted_times:
            ...
            for etype in events:
                while (
                    idx[etype] < len(events[etype])
                    and events[etype][idx[etype]][0] == t
                ):
        ...
        ranks = {"order": 1001, "pick": 1002, "return": 1003, "fetch": 1004}
        ...
        fig.update_yaxes(
            title_text="Station Util", row=util_row, col=1, showgrid=True, zeroline=True
        )
    ...
    title_text = (
        f"Order Batching & Station Scheduling (makespan: {makespan})<br>"
    ...
    if y_order_by_row[1]:
        dummy_y = y_order_by_row[1][
            0
        ]  # Use the first category (which will be at the top or bottom)
    ...
    fig.add_vline(
        x=makespan, line_width=2, line_dash="dot", line_color="#555", row="all", col=1
    )
