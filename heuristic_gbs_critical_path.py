#!/usr/bin/env python3
"""
AutoStore Global Bin Scheduling (GBS) Heuristic (Critical Path Rule).

This is a wrapper around heuristic_gbs.py that defaults the scoring rule
to "critical_path" instead of "readiness_weighted".
"""
...
def solve_heuristic_instance(config: dict, return_raw: bool = False):
    """Run one GBS instance described by *config*, defaulting to critical_path."""
    ...
    gbs_rule = config.get("gbs_rule", "critical_path")
...
def main() -> None:
    """CLI entry point for running the GBS heuristic (Critical Path) standalone."""
    import sys
    import os
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    
    ap = argparse.ArgumentParser(
        description="Global Bin Scheduling (GBS) Heuristic for AutoStore Task B [Critical Path Default]"
    )
