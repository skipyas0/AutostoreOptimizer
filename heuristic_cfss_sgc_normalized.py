...
def run_cfss_sgc(instance, *args, **kwargs) -> Solution:
    ...
    if not isinstance(instance, Instance):
        ...
        max_cluster_orders = kwargs.get(
            "max_cluster_orders", args[13] if len(args) > 13 else 8
        )
        ...
        max_cluster_orders = kwargs.get(
            "max_cluster_orders", args[5] if len(args) > 5 else 8
        )
    ...
    for o in O:
        req = req_sets[o]
        sharing_degree[o] = sum(
            1 for o2 in O if o2 != o and not req.isdisjoint(req_sets[o2])
        )
    ...
    if failed_orders:
        print(
            f"[CFSS-SGC] WARNING: {len(failed_orders)} orders could not be scheduled: {failed_orders}"
        )
