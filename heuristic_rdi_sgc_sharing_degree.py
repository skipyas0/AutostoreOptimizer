#!/usr/bin/env python3
"""
AutoStore Regret-Based Dynamic Insertion SGC (Sharing Degree Tiebreaker).

This is a wrapper around heuristic_rdi_sgc.py that defaults the tiebreaker
to "sharing_degree" instead of "sum_rt_asc".
"""
...
def solve_heuristic_instance(config: dict, return_raw: bool = False):
    """Run one RDI-SGC instance described by *config*, defaulting to sharing_degree."""
    ...
    tiebreaker = config.get("tiebreaker", "sharing_degree")
...
def main() -> None:
    """CLI entry point for running the RDI-SGC heuristic (Sharing Degree) standalone."""
    import sys
    import os
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    
    ap = argparse.ArgumentParser(description="Regret-Based Dynamic Insertion SGC [Sharing Degree Default]")
    ...
    ap.add_argument("--tiebreaker", type=str, choices=["sharing_degree", "best_score", "sum_rt_asc"], default="sharing_degree")
